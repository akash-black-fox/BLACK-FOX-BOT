...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "AKASH HASAN",
  description: "Create a friendship photo with someone",
  commandCategory: "Fun",
  usages: "friendpair [@mention]",
  cooldowns: 10,
  prefix: true
};
...
│
╰─────────────────────╯
বন্ধু মানে হাত বাড়িয়ে দেওয়া, আর বলা "চিন্তা করিস না আমি আছি তো"...!! 🤝💖`;

  try {
    const cacheDir = path.join(__dirname, "cache");
    if (!fs.existsSync(cacheDir)) fs.mkdirSync(cacheDir);
    
    const templatePath = path.join(cacheDir, "friendpair_bg.jpg");
    const outputPath = path.join(cacheDir, `friendpair_${senderID}.png`);
    ...
    const canvas = createCanvas(474, 317); 
    ...
    ctx.save();
    ctx.beginPath();
    ctx.arc(122, 157, 62, 0, Math.PI * 2, true); 
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(avatar1, 60, 95, 125, 125); 
    ctx.restore();

    ctx.save();
    ctx.beginPath();
    ctx.arc(347, 157, 62, 0, Math.PI * 2, true); 
    ctx.closePath();
    ctx.clip();
    ctx.drawImage(avatar2, 285, 95, 125, 125);
    ctx.restore();
